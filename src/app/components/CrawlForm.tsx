...
import { ChevronDown, CheckIcon, ImageIcon, WholeWordIcon, File } from "lucide-react";
...
export default function CrawlForm() {
  ...
  const [crawlType, setCrawlType] = useState("Keyword");

  const crawlTypes = [
    { type: "Keyword", icon: <WholeWordIcon /> },
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <form onSubmit={handleSearch}>
        <div className="relative max-w-full mx-auto mt-10 mb-3 flex gap-4">
          ...
          <div className="relative max-w-full mx-auto flex gap-4">
            <div className="flex-1">
              <input
                ...
                placeholder={`Enter ${crawlType.toLowerCase() === "keyword" ? crawlType.toLowerCase() : `${crawlType.toLowerCase()} phrase`} to search`}
            ...
            <div className="pt-5">
              ...
              {isDropdownOpen && (
                <ul className="absolute z-10 mt-1 max-h-56 overflow-auto rounded-md bg-white py-1 text-base ring-1 shadow-lg ring-black/5 focus:outline-none sm:text-sm">
        ...
        <div className="flex gap-3 justify-center items-center">
          <button
            type="submit"
            className={`flex-1 mt-5 px-4 py-2 w-full rounded-lg text-white ${isValidUrl && additionalText.trim() && crawlType !== "Select Type" ? "bg-blue-500" : "bg-gray-400 cursor-not-allowed"}`}
            disabled={!isValidUrl || !additionalText.trim() || crawlType === "Select Type" || loading}
          >
            {loading && <div>
